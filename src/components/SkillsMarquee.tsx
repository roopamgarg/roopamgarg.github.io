    ...
    <div className="relative w-full overflow-hidden bg-bg py-5 border-y border-border/5 select-none my-6">
      ...
      <div className="animate-marquee flex items-center gap-12 whitespace-nowrap">
        {itemsDouble.map((skill, index) => (
          <div
            key={`${skill}-${index}`}
            className="flex items-center gap-6 font-black text-xs sm:text-sm tracking-[0.25em] capitalize text-accent/80 transition-colors hover:text-accent duration-300"
          >
            <span>{skill}</span>
